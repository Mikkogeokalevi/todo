...
// --- LISÄTTY: Avain offline-tallennukselle ---
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- UUSI FUNKTIO: Ladataan tila selaimen muistista ---
    const loadStateFromOffline = () => {
        ...
        if (offlineData) {
            ...
            // Renderöidään heti offline-datalla, jotta näkymä ei ole tyhjä
            render();
            renderLoggers();
            renderFoundList();
        }
        ...
        try {
            ...
            if (currentMunicipality && currentMunicipality !== lastCheckedMunicipality) {
                ...
                if (foundMunIndex !== -1) {
                 ...
                 if (foundMunIndex !== -1) {
                    const munElement = document.getElementById(`mun-item-${foundMunIndex}`);
                    if(munElement) activeMunicipalityContainer.innerHTML = munElement.innerHTML;
                    activeMunicipalityContainer.classList.remove('hidden');
                 }
            }
    ...
    // --- MUOKATTU FUNKTIO: render (värikoodaus) ---
    const render = () => {
    ...
    loadStateFromOffline(); // Ladataan offline-data heti alussa
    initMap();
    ...
    // --- MUOKATTU FUNKTIO: saveState (lisätty offline-tallennus) ---
    const saveState = () => {
        const dataToSave = {
            municipalities,
            foundCaches,
        ...
        // Tallennetaan aina myös selaimen muistiin
    ...
    globalAddFromPgcBtn.addEventListener('click', async () => {
        ...
        for (let i = 0; i < lines.length; i += 3) {
            ...
            try {
                ...
                if (munIndex === -1) { 
                    municipalities.push({ name: munName, caches: [cacheData], lat: coords.lat, lon: coords.lon, hadCaches: true }); // LISÄTTY hadCaches
                } else { 
                    if (!municipalities[munIndex].caches) municipalities[munIndex].caches = [];
                    municipalities[munIndex].caches.push(cacheData);
                    municipalities[munIndex].hadCaches = true; // LISÄTTY hadCaches
                }
                cachesAddedCount++;
