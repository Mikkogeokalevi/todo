...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // KORJATTU: Funktio tunnistaa nyt sekä DDM että DD -muodot
    const parseCoordinates = (str) => {
        if (!str) return null;
        let cleaned = str.toString().trim().toUpperCase().replace(/,/g, '.').replace(/°|´|`|'/g, ' ');

        // Tunnista DD (Decimal Degrees) -muoto: esim. 60.9582 25.7694
        ...
        // Tunnista DDM (Degrees Decimal Minutes) -muoto
    ...
    const ensureAllCoordsAreFetched = async (munis) => {
        ...
        if (didChange) {
            ...
            saveMunicipalities();
            return true; // Palautetaan tieto muutoksesta
        }
    ...
    onValue(ref(database, 'paalista'), async (snapshot) => {
        ...
        render(); // Ensimmäinen renderöinti
        const changed = await ensureAllCoordsAreFetched(municipalities);
        if (changed) render(); // KORJATTU: Uudelleenrenderöinti, jos koordinaatteja lisättiin
        updateAllMarkers();
    });
    ...
    municipalityList.addEventListener('click', (e) => {
        // Sisältö pysyy samana, mutta parseDDMCoordinates on nyt parseCoordinates
        ...
        if (button.classList.contains('set-coords-btn')) {
            ...
            const coords = parseCoordinates(input); // KÄYTETÄÄN UUTTA FUNKTIOTA
            if(coords) {
