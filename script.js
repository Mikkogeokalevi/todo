...
document.addEventListener('DOMContentLoaded', () => {
    ...
    onValue(ref(database, FIREBASE_PATH), async (snapshot) => {
        ...
        // `onValue` on AINOA paikka, joka päivittää käyttöliittymän datan perusteella
    ...
    globalAddFromPgcBtn.addEventListener('click', async () => {
        ...
        for (let i = 0; i < lines.length; i += 3) {
            ...
            try {
                ...
                if (munIndex === -1) { 
                    municipalities.push({ name: munName, caches: [cacheData], lat: coords.lat, lon: coords.lon });
                } else { 
    ...
    addLoggerBtn.addEventListener('click', () => {
        ...
        if (newName && !loggers.find(l => l.toLowerCase() === newName.toLowerCase())) {
            ...
            // ÄLÄ renderöi tässä. Annetaan `onValue`-kuuntelijan hoitaa se,
            // kun se saa vahvistuksen tietokannasta.
            saveState();
        }
    });
    ...
    loggerList.addEventListener('click', (e) => {
        if (e.target.classList.contains('delete-logger-btn')) {
            ...
            // ÄLÄ renderöi tässä. `onValue` hoitaa.
            saveState();
        }
    });
