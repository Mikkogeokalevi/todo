...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleMapClick = async (e) => {
        ...
        try {
            ...
            if (municipalityName) {
                const isOnList = municipalities.some(m => m.name.toLowerCase() === municipalityName.toLowerCase());
                if (isOnList) {
    ...
    const toggleTracking = async () => {
        ...
        // Lisätty tarkistus, että rajakerros poistetaan, kun seuranta aloitetaan/lopetetaan
        if (currentBoundaryLayer) {
            map.removeLayer(currentBoundaryLayer);
            currentBoundaryLayer = null;
        }
